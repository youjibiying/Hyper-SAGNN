...
def parse_args():
    ...
    parser.add_argument('--data', type=str, default='GPS')
...
def train_epoch(args, model, loss_func, training_data, optimizer, batch_size, only_rw, type):
    ...
    bar = trange(batch_num, mininterval=0.1,
                 desc='  - (Training) ', leave=False, )
    for i in bar:
        if only_rw or alpha > 0:  # inly random walk
            ...
            neg_samples = torch.tensor(
                neg_samples, dtype=torch.long, device=device)
            ...
            batch_edge_weight = edge_weight[i *
        ...
        for opt in optimizer:
            opt.step()
        # print(">>>>>MSEloss:", bce_total_loss / (i + 1),loss_bce.item())
    ...
    # r_squres(决定系数)，mae:mean_absolute_error 平法绝对误差
    auc1, auc2 = r2_mae_cuda(y, pred)
...
def eval_epoch(args, model, loss_func, validation_data, batch_size, type):
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(int(math.floor(len(validation_data) / batch_size))),
            ...
            try:
                loss = loss_func(pred_batch, batch_y,
                                 weight=pred_batch)  # 给loss指定权重，交叉熵损失
            except:
        ...
        acc = -1  # accuracy(pred, label)

        # r_squres(决定系数)，mae:mean_absolute_error 平方绝对误差
        auc1, auc2 = r2_mae_cuda(label, pred)
...
def train(args, model, loss, training_data, validation_data, optimizer, epochs, batch_size, only_rw):
    ...
    for epoch_i in range(epochs):
        ...
        print('  - (Training)   bce: {bce_loss: 7.4f}, skipgram: {skipgram_loss: 7.4f}, '
              ...
              ' acc: {accu:3.3f} %, r_score: {auc1:3.3f}, mae: {auc2:3.3f}, '
...
def generate_negative(x, dict1, get_type='all', weight="", forward=True):  # 生成负样本（也就是构建不存在的边）
    if dict1 == 'train_dict':
        dict1 = train_dict
    ...
    if forward:
        func1 = pass_  # return x
    else:
        func1 = tqdm

    if len(x.shape) > 1:  # x.shape:[96,3]
    ...
    for j, sample in enumerate(func1(x)):
        ...
        for i in range(neg_num):  # 基于原来的正样本 sample 来改变其中的1 至多条边来生成neg_num=5个负样本
            ...
            while not a.isdisjoint(dict1):  # a 不在以前的dict1（正例）set 中
                temp = np.copy(sample)
                trial += 1
                if trial >= 1000:
                    temp = ""
                ...
                if simple_or_hard <= pair_ratio:  # pair_ratio 就是保持2个node 不变，一个node 变的比例
                    if len(num_list) == 1:
                        # Only one node type
                        ...
                        temp[change] = np.random.randint(
                            int(start), int(end), 1) + 1  # 对temp(也就是原本的边)改变指定的第change index 上的边。
                else:
...
def save_embeddings(model, origin=False):
    ...
    with torch.no_grad():
        ...
        embeddings = []
        for j in range(math.ceil(len(ids) / batch_size)):  # 当batchsize =96 时
            # 取batch size 对应的下标
            ...
            # 每个batch 通过autoencode 和layernorm 生成的embed 。
            embeddings.append(embed)

        embeddings = np.concatenate(embeddings, axis=0)[
            :, 0, :]  # 去掉中间维度 [221,64] 221 为顶点总数
        for i in range(len(num_list)):
            start = 0 if i == 0 else num_list[i - 1]
            static = embeddings[int(start):int(num_list[i])]
            np.save("../model/hyper-sagnn/mymodel_%d.npy" % (i), static)

            if origin:
                np.save("../model/hyper-sagnn/mymodel_%d_origin.npy" %
                        (i), static)
...
def generate_embeddings(edge, nums_type, H=None, weight=1):
    ...
    embeddings = [H[i].dot(s_vstack([H[j] for j in range(len(num))]).T).astype('float32') for i in
                  range(len(nums_type))]  # shape=[[146,221],[70,221],[5,221]]
    ...
    for i, e in enumerate(embeddings):
        ...
        new_e = e[:, col_sum > 0]
        new_e.eliminate_zeros()  # Remove zero entries from the matrix
...
pair_ratio = 0.9  # 负样本中，保持2个node 不变，一个node 变的比例
train_type = 'hyper'
...
try:
    train_weight, test_weight = train_zip["train_weight"].astype(
        'float32'), test_zip["test_weight"].astype('float32')
...
if len(num) > 1:
    node_type_mapping = [0, 1, 2]  # 三种节点类型，用于


if args.feature == 'adj':  # 用当前类型的node与其他类型node 的adj作为feature ，
    # embedding size [[146,75],[70,151],[5,216]]
    embeddings_initial = generate_embeddings(
        train_data, num, H=None, weight=train_weight)
...
def cut_fun_label(train_data, test_data):
    ...
    cut_value = np.array(cut_value, dtype=np.float)/100
...
train_dict = parallel_build_hash(
    train_data, "build_hash", args, num, initial=set())
test_dict = parallel_build_hash(
    test_data, "build_hash", args, num, initial=train_dict)
    ...
    with tf.device("/cpu:0"):
        if args.feature == 'walk':
            ...
            if not args.TRY and os.path.exists(
                ...
                wv = w2v.wv
                A = [wv[str(i)] for i in range(int(num_list[-1]))]
            ...
            node_embedding = MultipleEmbedding(
                ...
                flag,
                num_list,
                node_type_mapping).to(device)  # 将intial embeddings 经过autoencoder 生成node embbeding
        ...
        classifier_model = Classifier(
            ...
            d_k=16,  # dim of key
            d_v=16,  # dim of value
            node_embedding=node_embedding,  # 将处理过的node emnbdding 输入到classifier中
            diag_mask=args.diag,
        ...
        params_list = list(
            set(list(classifier_model.parameters()) + list(Randomwalk_Word2vec.parameters())))
        ...
        train(args, (classifier_model, Randomwalk_Word2vec),
              ...
              optimizer=[optimizer], epochs=args.epochs, batch_size=batch_size, only_rw=False)  # 轮
